	...
	class General extends Connection{
		...
		{ 
			if(isset($_REQUEST['staff_type'])){    
				...
				//INSERT SECTION
				$sql = "update  essort_user_header set usr_fname='".$txtName."',usr_mname='".$txtMName."',usr_lname='".$txtLName."',usr_role='".$staff_type."',usr_email='".$txtEmail."',usr_status='1',usr_mobile='".$txtContact."' WHERE usr_id='".$staff_id."'"; 
				$resp = mysql_query($sql);
				//$staff_id = mysql_insert_id();
				//DETAIL 
				if ($resp==true)
		...
		{ 
			if(isset($_FILES["sal_file"]["tmp_name"]) ){    
				...
				$filename=$_FILES["sal_file"]["tmp_name"];
				$month=$_POST['month'];
				$year=$_POST['sal_year'];
        ...
        function markHoliday(){
            $markholidays = mysql_query("SELECT * FROM essort_holidays WHERE DATE_FORMAT(off_day,'%Y') = '".date('Y')."'");
        ...
        function getEventsNotification(){
            ...
            AND DATE_FORMAT( `off_day` , '%Y-%m-%d' ) >= '" . date('Y-m-d') . "' GROUP BY occassion LIMIT 5") OR DIE(mysql_error());
		...
		$sqlleave=mysql_query("SELECT *,leave_date as maxdate,leave_date as mindate,'USERFNAME','USERLNAME' FROM essort_teacher_leave_info WHERE leave_date='".date('Y-m-d')."'");
		...
		{ 
			if(isset($_REQUEST['leave_id']) ){    
				...
				$sqlupdate=mysql_query("update  essort_teacher_leave_info set leave_status='Confirmed' WHERE leave_id='".$leave_id."'");
				if($sqlupdate==true){
					return 1;
				}else{
					return 0;
       ...
       //ALL STAFF WITH ATTENDENCE
        ...
        ################################FOR STUDENTS#################################
		function ALLSTUWITHFEE(){
       $stusql=mysql_query("SELECT *, att_ref_id,'USERFNAME','USERLNAME','att_status','att_intime','att_outtime' ,'app_no','att_id','amount','last_payment','last_date' FROM essort_user_relation"); 
       ...
       ##################################################################
	    function VIEWALLMESSAGE($id,$role,$page,$type){
                if(isset($id)){
				$num_rec_per_page='20';
                    ...
                    $sql = mysql_query("SELECT *,'messages','from_name' FROM essort_message_master WHERE to_id='" . $id . "' AND to_role='" . $role . "' $cond GROUP BY subject LIMIT $start_from,$end");
				   $arr=array();
	   ...
	   else
	   {
			...
			$sqlholidays=mysql_query("SELECT * FROM essort_holidays WHERE off_day='".$Date."'");
