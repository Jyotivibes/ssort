                        ...
                        <table class="table table-hover table-bordered">
						  ...
						  ?>
							...
							{  
								...
								{
									$id=$termvalue['exam']."text";
								}

							?>
                               <td><input id="<?php echo $id."".$subvlaue['sub_id'];?>" type="text" class="form-control classval"  onkeypress="return isNumber(event)"></td>
							<?php
...
$(".classval").keyup(function(){
		 var id = $(this).attr('id');
		 //var id = id.split('text')[0]+'text';
		//alert(id);
		 var mynumber=$("#"+id).val();
		 var sub = id.split('text')[1];
		 var id = id.split('text')[0];
		 //var text = "text";
		 setCookie("subject", sub, 365);
		 alert(getCookie("subject"));
		 
		 
		  
		 //id = id.replace(text,'');
		 //var marks="marks";
		 // var res = id.concat(marks);
		 //var hash="#";
		 //var ids = hash.concat(res);
		 //var main=$(ids).html();
		  //alert(["FA1", "FA2", "SA1"].includes(id));
		 if(["FA1", "FA2", "SA1"].includes(id)==true)
		 {
			window.total = parseInt(mynumber)+parseInt(total);
		 
		 }
		 if(["FA3", "FA4"].includes(id)==true)
		 {
			window.total2 = parseInt(mynumber)+parseInt(total2);
		 
		 }
		 
		 $("#TotalTerm1"+sub+"").val(total);
		 $("#TotalTerm2"+sub+"").val(total2);
		 var grandtotal=parseInt(total)+parseInt(total2);
		 $("#gradtotal").val(grandtotal);
