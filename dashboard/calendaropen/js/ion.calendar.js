        ...
        init: function(options){
            ...
            return this.each(function(){
                ...
                var placeCalendar = function(){
                    ...
                    var len=markHolidays.length;
                    var lenInner;
                       for(i=0;i<len;i++)
                        {
                            for(j=0;j<markHolidays[i].holidays.length;j++)
                                {
                                    $day.each(function(){
                                        // Jyoti: We have included 'year' parameter here.
										//if(parseInt($(this).text())== markHolidays[i].holidays[j].day && parseInt($month.val())==markHolidays[i].month){
                                        if(parseInt($(this).text())== markHolidays[i].holidays[j].day && parseInt($month.val())==markHolidays[i].month && parseInt($year.val())==markHolidays[i].year){
                                         $(this).css({
                                             "background-color":"red",
                                             "color":"white"
                                         });
                                            $(this).attr("title",markHolidays[i].holidays[j].holiday);
                                        }
                                    });
                                }
                        }
